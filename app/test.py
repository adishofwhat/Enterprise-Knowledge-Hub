...
if search_query:
    with st.spinner("Searching..."):
        ...
        if results:
            ...
            for idx, result in enumerate(results):  # Use enumerate to generate a unique index
                ...
                # Add unique key to the download button
                st.download_button(
                    label="View Full Document",
                    data=full_text,
                    file_name=doc_id,
                    key=f"download_{doc_id}_{idx}"  # Unique key for each button
                )
